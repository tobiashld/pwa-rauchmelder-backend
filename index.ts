...
app.post("/logout", (req, res) => {
	res
		...
		.status(200)
		.json({ error: "Erfolgreich ausgeloggt", status: 290 });
});
...
app.get(
	"/pruefungen",
	auth.authenticateToken,
	async (req: { query: { id: string } }, res: any) => {
		try {
			if (Object.keys(req.query).length <= 0) {
...
app.post(
	"/rauchmelder/switch/create",
	async (
		req: {
			body: {
				newRauchmelder: Rauchmelder;
				altRauchmelderBz: RauchmelderBeziehung;
			};
		},
		res
	) => {
...
app.get("/rauchmelder/objekt/:id", async (req, res) => {
	...
	rauchmelder
		.getAllWithObjectId(req, res, Number.parseInt(req.params.id))
		.catch((err) => {
			res
				.status(200)
				.json({
					status: 406,
					error: "Verändern des Rauchmelders fehlgeschlagen",
				});
		});
});

app.get(
	"/rauchmelder/variant/:variante",
	auth.authenticateToken,
	async (req, res) => {
		var rauchmelderhistorienid = req.params.variante;
		rauchmelder
			.getWithHistoryId(req, res, rauchmelderhistorienid)
			.catch((err) => {
				res
					.status(200)
					.json({
...
app.get(
	"/rauchmelder/history/:variante",
	auth.authenticateToken,
	(req, res) => {
		var rauchmelderhistorienid = req.params.variante;
		rauchmelder
			.getHistory(req, res, Number.parseInt(rauchmelderhistorienid))
			.catch((err) => {
				res
					.status(200)
					.json({
...
app.delete(
	"/rauchmelder/:id",
	auth.authenticateToken,
	async (req: { params: { id: any } }, res) => {
		var rauchmelderid = req.params.id;
		rauchmelder.deleteRauchmelder(req, res, rauchmelderid).catch((err) => {
			res
				.status(200)
				.json({ error: "Löschen des Rauchmelders fehlgeschlagen" });
		});
	}
);

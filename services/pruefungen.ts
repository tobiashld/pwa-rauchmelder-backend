import { PruefungenListe, Rauchmelder } from "../types/rauchmelder";
import db from "./db";
...
function getAllMapping(
	...
	response: {
		status: (arg0: number) => {
			(): any;
			new (): any;
			json: { (arg0: { data: any[] }): void; new (): any };
		};
	}
) {
			...
			r: {
				[x: string]: {
					rauchmelder: {
						...
						rauchmelderId: any;
						selberRaum: any;
						baulichUnveraendert: any;
						hindernisseUmgebung: any;
						relevanteBeschaedigung: any;
						oeffnungenFrei: any;
						warnmelderGereinigt: any;
						pruefungErfolgreich: any;
						batterieGut: any;
						grund: any;
						anmerkungen: any;
						anmerkungenZwei: any;
...
async function getAllWithParam(
	...
	const query =
		'SELECT pruefungen.timestamp as pruefungszeit,pruefungen.*,"pruefungenListe".*,users.*,objekte.* FROM pruefungen Join "pruefungenListe" On "pruefungenListe"."pruefungsID" = pruefungen.id Join users On users.user_id = pruefungen."userID" Join objekte On pruefungen."objektID" = objekte.id WHERE ' +
		key +
		" = " +
		value;
		...
		db.prisma.pruefungen
			.create({
				data: {
					...
					pruefungenListe: {
						createMany: {
							data: pruefung.rauchmelder.map((item: any) => {
								return { ...item, rauchmelderhistorie: undefined };
							}),
						},
						// createMany:{
						//     data:[{}],

...
function onMessage(ws: any, payload: any, message: string) {
	let deserializedMessage: { chat: string; message: string; from: string } =
		JSON.parse(message);

	db.prisma.nachrichten
		.create({
			data: {
				chat: {
					connectOrCreate: {
		...
		.then((nachricht: any) => {
			db.prisma.chatteilnehmer
				...
				.then((teilnehmer: any) => {
					let sender = teilnehmer.find(
						(einzel: any) => einzel.userid === payload.id
					)?.user;
					console.log("nachricht gespeichert und wird geschickt");
					teilnehmer.forEach((einzelTeilnehmer: any) => {
						if (
							clients[einzelTeilnehmer.userid] &&
							einzelTeilnehmer.userid !== payload.id
						) {
							clients[einzelTeilnehmer.userid].send(
								JSON.stringify({
									stage: 2,
									data: {
										nachricht: deserializedMessage.message,
										chat: deserializedMessage.chat,
										user: { username: sender?.username, email: sender?.email },
									},
								})
							);
...
function onSupport(ws: any, userUUID: string, message: string) {
	setTimeout(
		() =>
			ws.send(
				JSON.stringify({
					stage: 2,
					data: "Dies ist eine Automatische Antwort von unserem Chatbot! Ihr Anliegen wird direkt an den Support weitergegeben",
				})
			),
		1000
	);
	db.prisma.chats
		...
		.then((chats: any) => {
			let id =
				chats.length > 0 ? chats[0].id : "00000000-0000-0000-0000-000000000000";
			db.prisma.nachrichten
				.create({
					data: {
						nachricht: message,

...
async function getAllWithParams(request, response,params){
    ...
    const paramsQuery = Object.keys(params).map(key=>`objekte."`+key.toString()+`" =`+(isNaN(params[key])?` '${params[key]}'`:` ${params[key]}`)).join(" AND ");
...
async function changeObjekt(request,response,objektid){
  var objekt = request.body
  const q = `UPDATE public.objekte SET `+mapObjectToParams(objekt)+` WHERE id=${objektid};`;
  db.query(
    q,
    response
    )
